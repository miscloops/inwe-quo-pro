...
function parseCreds(raw: string): { username: string; password: string }[] {
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      if (!seen.has(username)) { seen.add(username); creds.push({ username, password }) }
    }
  }
  return creds
}
  ...
  const loginMutation = useMutation({
    mutationFn: async () => {
      ...
      for (let i = 0; i < creds.length; i++) {
        ...
        try { await loginOne(username, password); okCount++ }
        catch (e) { failed.push(`${username}: ${e instanceof Error ? e.message : 'failed'}`) }
    ...
    onSuccess: ({ okCount, failed }) => {
      ...
      if (failed.length === 0) toast.success(`All ${okCount} account(s) logged in successfully`)
      else if (okCount > 0) toast.warning(`${okCount} logged in, ${failed.length} failed`, { description: failed.slice(0, 3).join('\n') })
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Label htmlFor="show-pass" className="text-xs text-[#666] cursor-pointer">Show</Label>
          ...
          <Textarea
            ...
            placeholder={example}
            className={`min-h-[140px] font-mono text-xs bg-[#f5f5f0] ${showPasswords ? '' : '[-webkit-text-security:disc]'}`}
        ...
        {progress && (
          <div className="border border-[#999] bg-[#f5f5f0] p-2 text-xs">
            ...
            <div className="mt-1.5 h-1.5 overflow-hidden bg-[#e8e8e0]">
              <div className="h-full bg-[#0066cc] transition-all" style={{ width: `${(progress.done / progress.total) * 100}%` }} />
            </div>
            <div className="mt-1 flex gap-3">
              <span className="text-green-600">✓ {progress.ok}</span>
              <span className="text-red-600">✗ {progress.failed.length}</span>
            </div>
          </div>
        )}
        <div className="flex items-center gap-2">
