...
export function AutoGiftingPanel({ sessions }: AutoGiftingPanelProps) {
  ...
  const confirmStartWithPin = async () => {
    ...
    try {
      ...
      // Start the polling loop — calls /start every 4s to send the next gift
      // (Cloudflare Workers are stateless: each /start request sends one gift)
      startGiftLoop()
  ...
  // Gift loop: repeatedly calls /api/inwe/auto-gift (action: 'start') every 4 seconds.
  // Each request: re-login sender → join room via Socket.IO → send one gift via REST.
  // Always passes password + force_relogin=true because Workers are stateless.
  ...
  const startGiftLoop = useCallback(() => {
    ...
    const sendNextGift = async () => {
      ...
      try {
        const r = await fetch('/api/inwe/auto-gift', {
          ...
          body: JSON.stringify({
            ...
            force_relogin: true,  // always re-login — Workers are stateless
          }),
        })
        const j = await r.json()

        // Add new log entries to the UI
        if (j.ok && j.log) {
        ...
        if (j.status === 'failed') {
          ...
          // Wait 6s before retrying (longer than normal 4s)
      ...
      // Wait 4 seconds before sending the next gift
      if (!cancelled && runningRef.current) {
        setTimeout(sendNextGift, 4000)
      }
    }

    // Don't send immediately — the initial /start already sent the first gift
    setTimeout(sendNextGift, 4000)
    ...
    <div className="border-2 border-[#666] bg-[#fff]">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className={`flex items-start gap-2 border p-2 text-xs ${running ? 'border-green-400 bg-green-50' : 'border-orange-400 bg-orange-50'}`}>
          ...
          <div><p className="font-semibold">Gifting Guard: {running ? 'ACTIVE' : 'STANDBY'}</p><p className="text-[#666] mt-0.5">The bot will never send a gift to an ID at {GUARD_THRESHOLD}%+ progress.</p></div>
        </div>
        <div>
