...
export function AutoGiftingPanel({ sessions }: AutoGiftingPanelProps) {
  ...
  return (
    <div className="border-2 border-[var(--border)] bg-[var(--card)]">
      <div className="border-b border-[var(--border)] px-4 py-2 bg-[var(--header-bg)]">
        <div className="flex items-center gap-2">
          <Bot className="h-4 w-4 text-[var(--header-text)]" /><h2 className="text-sm font-bold">3. Auto Gifting Bot</h2>
        ...
        <p className="mt-0.5 text-[10px] text-[var(--muted-foreground)]">Bot enters QUO PRO chatroom and cycles 5 gift commands (neko, ganja, fly, muri, best wishes) round-robin at 4-second intervals.</p>
      </div>
      <div className="p-4 space-y-3">
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
          <div className="space-y-1.5"><Label className="text-xs">Sender account</Label><Select value={effectiveSender} onValueChange={setSender} disabled={running}><SelectTrigger className="bg-[var(--muted)]"><SelectValue placeholder="Select sender..." /></SelectTrigger><SelectContent>{sessions.map(s => <SelectItem key={s.id} value={s.username}>{s.username} {s.level !== null ? `(L${s.level})` : ''}</SelectItem>)}</SelectContent></Select></div>
          <div className="space-y-1.5 sm:self-end">{running ? <Button onClick={stopBot} variant="destructive" className="w-full sm:w-auto"><Square className="mr-2 h-4 w-4" /> Stop</Button> : <Button onClick={startBot} disabled={pendingStart || sessions.length < 2} className="w-full sm:w-auto bg-[var(--primary)] text-[var(--primary-foreground)]"><Play className="mr-2 h-4 w-4" /> Start Auto Gifting</Button>}</div>
        </div>
        {effectiveSender && (
          <div className="grid gap-2 sm:grid-cols-3 text-xs">
            <div className="border border-[var(--border)] bg-[var(--muted)] p-2"><p className="text-[var(--muted-foreground)]">Recipients</p><p className="font-semibold tabular-nums">{activeCount} active / {recipients.length} total</p></div>
            <div className="border border-[var(--border)] bg-[var(--muted)] p-2"><p className="text-[var(--muted-foreground)]">Guarded (≥{GUARD_THRESHOLD}%)</p><p className="font-semibold tabular-nums text-orange-500">{guardedCount}</p></div>
            <div className="border border-[var(--border)] bg-[var(--muted)] p-2"><p className="text-[var(--muted-foreground)]">Gift commands</p><p className="font-semibold tabular-nums">{GIFT_COMMANDS.length} (cycling)</p></div>
          </div>
        )}
        <div className="space-y-1.5">
          ...
          <div className="border border-[var(--border)] bg-[var(--muted)] p-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs font-mono">
              {GIFT_COMMANDS.map((cmd, i) => <div key={cmd} className="flex items-center gap-2 bg-[var(--card)] px-2 py-1 border border-[var(--border)]"><span className="text-[var(--muted-foreground)] w-5">#{i + 1}</span><code className="text-[var(--primary)]">/gift &lt;username&gt; {cmd}</code></div>)}
            </div>
          </div>
        </div>
        <div className={`flex items-start gap-2 border p-2 text-xs ${running ? 'border-green-400 bg-green-50' : 'border-orange-400 bg-orange-50'}`}>
          ...
          <div><p className="font-semibold">Gifting Guard: {running ? 'ACTIVE' : 'STANDBY'}</p><p className="text-[var(--muted-foreground)] mt-0.5">Skip IDs at {GUARD_THRESHOLD}%+ progress. Switch to next recipient automatically. Auto-refresh progress every 60 seconds.</p></div>
        </div>
        <div>
          <div className="mb-1.5 flex items-center justify-between"><Label className="text-xs flex items-center gap-1.5"><Activity className="h-3.5 w-3.5" /> Bot activity log</Label>{logs.length > 0 && <button onClick={() => setLogs([])} className="text-[10px] text-[var(--muted-foreground)] hover:text-[var(--foreground)]">Clear</button>}</div>
          <div className="max-h-48 overflow-y-auto border border-[var(--border)] bg-[var(--muted)] p-2 font-mono text-[11px] space-y-0.5">
            {logs.length === 0 ? <p className="text-[var(--muted-foreground)] italic">No activity yet. Click Start Auto Gifting to begin.</p>
            : logs.map(l => <div key={l.id} className="flex items-start gap-2"><span className="text-[var(--muted-foreground)] shrink-0">{l.ts}</span><span className="shrink-0">{l.kind === 'success' && <CheckCircle2 className="h-3 w-3 text-green-500" />}{l.kind === 'skip' && <Shield className="h-3 w-3 text-orange-500" />}{l.kind === 'error' && <XCircle className="h-3 w-3 text-red-500" />}{l.kind === 'guard' && <Shield className="h-3 w-3 text-[var(--primary)]" />}{l.kind === 'info' && <Activity className="h-3 w-3 text-[var(--muted-foreground)]" />}</span><span className={l.kind === 'success' ? 'text-green-700' : l.kind === 'skip' ? 'text-orange-700' : l.kind === 'error' ? 'text-red-700' : l.kind === 'guard' ? 'text-[var(--primary)]' : ''}>{l.msg}</span></div>)}
          </div>
        </div>
        {recipients.length > 0 && (
          <div><Label className="text-xs">Recipients (live status)</Label><div className="mt-1.5 grid gap-1.5 sm:grid-cols-2">
            {recipients.map(r => { const pct = r.pointPct ?? 0; const guarded = pct >= GUARD_THRESHOLD; return (
              <div key={r.id} className={`flex items-center gap-2 border p-2 text-xs ${guarded ? 'border-orange-400 bg-orange-50' : 'border-[var(--border)] bg-[var(--muted)]'}`}>
                <div className="flex h-7 w-7 items-center justify-center bg-[var(--secondary)] text-[10px] font-bold text-[var(--primary)]">{r.username.slice(0, 2).toUpperCase()}</div>
                <div className="min-w-0 flex-1"><p className="truncate font-medium">{r.username}</p><div className="mt-0.5 h-1 overflow-hidden bg-[var(--secondary)]"><div className={`h-full ${guarded ? 'bg-orange-500' : 'bg-[var(--primary)]'}`} style={{ width: `${Math.min(pct, 100)}%` }} /></div></div>
                <div className="text-right shrink-0"><p className={`font-semibold tabular-nums ${guarded ? 'text-orange-500' : ''}`}>{pct.toFixed(1)}%</p>{r.level !== null && <p className="text-[9px] text-[var(--muted-foreground)]">L{r.level}</p>}</div>
      ...
      <Dialog open={pinDialogOpen} onOpenChange={(o) => { setPinDialogOpen(o); if (!o && pendingStart) setPendingStart(false) }}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Shield className="h-4 w-4 text-[var(--primary)]" /> Start Auto Gifting</DialogTitle><DialogDescription>Enter QUO PRO chatroom and start sending 5 gift commands every 4 seconds. Enter the sender&apos;s password below.</DialogDescription></DialogHeader>
          ...
          <DialogFooter><Button variant="ghost" onClick={() => { setPinDialogOpen(false); setPendingStart(false) }}>Cancel</Button><Button onClick={confirmStartWithPin} disabled={pin.length < 4} className="bg-[var(--primary)] text-[var(--primary-foreground)]"><Play className="mr-2 h-4 w-4" /> Start Bot</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  )
